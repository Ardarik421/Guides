# Руководство пользователя Периметр-ПМ
...
Блок **Диагнозы, риски** включает предварительные и подтвержденные диагнозы, которые были добавлены медицинский работником во время приема или автоматически предложены системой а также риски, принятые медицинским работником во время приема либо рассчитанные системой автоматически.
...
Нажмите кнопку **Подписать и завершить**. В блок **История обращений** будет добавлен новый протокол проема.

##### План мероприятий

План мероприятий формируется только для протокола диспансерного приема врача-терапевта цехового участка.
...
В блок **История обращений** будет добавлен новый протокол проема.
...
Для создания новой группы компаний перейдите в раздел **Организации** и в правом верхнем углу нажмите кнопку **Создать**:
...
Для добавления локации откройте необходимую организацию и нажмите кнопку **Новая локация**:
...
Каждый импортируемый документ должен представлять собой файл формата `csv` с определенным набором данных. Содержимое файла — таблица, содержащая наименования и значения полей в соответствии с форматом импортируемого файла. Требования к форматам файлов представлены в следующих разделах Приложения 1.
...
Сохраните заполненную таблицу в формате `csv` и выполните импорт файла в соответствии с разделом **Импорт данных**.

> После сохранения таблицы в формат `csv`, убедитесь что в качестве разделителей значений используются запятые.
...
| title ( название)                                     | Не более128 знаков. Не должно быть пустым. Документ с данным полем может было загружен только один раз в каждой локации. | С заглавной буквы. Если название подразделения состоит из нескольких слов, требуется их разделить дефисом либо пробелом. Пример: **Департамент делопроизводства** |
...
| Поле                    | Требование                                                   | Комментарий                                                  |
| ----------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| СНИЛС                   | Одинадцатитизначная комбинация цифр, разделенных дефисом.    | Пример: **XXX-EEE-YYY ZZ**                                   |
| Табельный номер         | Четырехзначная комбинация цифр.                              | Пример: **1234**                                             |
...
| id подразделения        |                                                              | Берется из кадровой учетной системы предприятия.             |
...
| Дата приема на работу   | Дата вида "дд.мм.гггг".                                      | Пример: **02.05.1987**                                       |
| Адрес электронной почты | Должно быть заполнено в случае, если поле **Телефон** является пустым. Наличие в адресе символа "@" и точки в доменной части обязательно | Пример: **123@gmail.com**                                    |
...
| Поле                                          | Требование                                                   | Комментарий                                                  |
| --------------------------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| UID                                           | Одинадцатитизначная комбинация цифр, разделенных дефисом. Должно быть заполнено | СНИЛС. Пример: **XXX-EEE-YYY ZZ**                            |
...
| Необходимость доопследования (+/-)            |                                                              | В случае соответствия установите +. В случае несоответствия установите - |
...
Требования к формату импортируемого файла для загрузки данных учета временной утраты нетрудоспособности:

| Поле                           | Требование                                                   | Поле                                                         |
| ------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
...
| Номер больничного листа        | Должно быть заполнено. Не может дублироваться в одном файле  | Двенадцатизначнная комбинация цифр                           |
| Дата начала первого периода от | Дата вида "дд.мм.гггг". Должно быть заполнено. Не может быть больше даты загрузки файла | Пример: **02.05.1987**                                       |
| Дата оконч. перв. периода отсу | Дата вида "дд.мм.гггг". Должно быть заполнено. Не может быть больше даты загрузки файла | Пример: **02.05.1987**                                       |
| Дата начала второго периода от | Дата вида "дд.мм.гггг". Должно быть заполнено. Не может быть больше даты загрузки файла | Пример: **02.05.1987**                                       |
| Дата оконч. второго периода от | Дата вида "дд.мм.гггг". Должно быть заполнено. Не может быть больше даты загрузки файла | Пример: **02.05.1987**                                       |
| Дата нач. третьего периода отс | Дата вида "дд.мм.гггг". Должно быть заполнено. Не может быть больше даты загрузки файла | Пример: **02.05.1987**                                       |
| Дата оконч. третьего периода о | Дата вида "дд.мм.гггг". Должно быть заполнено. Не может быть больше даты загрузки файла | Пример: **02.05.1987**                                       |
| ПредБЛист                      | Не может дублироваться в одном файле                         | Двенадцатизначнная комбинация цифр. В случае указания номера предыдущего больничного листа, убедитесь что данный номер указан в отдельной записи как номер больничного листа |
| Номер следующего больничн. лис | Не может дублироваться в одном файле                         | Двенадцатизначнная комбинация цифр. В случае указания номера следующего больничного листа, убедитесь что данный номер указан в отдельной записи как номер больничного листа |
| Ид. номер                      | Одинадцатитизначная комбинация цифр, разделенных дефисом. Должно быть заполнено | СНИЛС. Пример: **XXX-EEE-YYY ZZ**                            |
...
| ---------------------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| ФИО                                      |                                                              | Кириллицей                                                   |
| Дата рождения                            | Дата вида "дд.мм.гггг"                                       | Пример: **02.05.1987**                                       |
| UID                                      | Должно быть заполнено. Одинадцатитизначная комбинация цифр, разделенных дефисом. | СНИЛС. Пример: **XXX-EEE-YYY ZZ**                            |
| ОГРН мед. организации                    | Должно быть заполнено. Тринадцатизначная комбинация цифр     | Пример: **1234567890123**                                    |
...
| Дата начала оказания услуги              | Дата вида дд.мм.гггг. Должно быть заполнено. Не может быть больше даты загрузки файла | Пример: **02.05.1987**                                       |
| Дата завершения оказания услуги          | Дата вида дд.мм.гггг. Должно быть заполнено. Не может быть больше даты загрузки файла | Дата вида дд.мм.гггг. Пример: **02.05.1987**                 |
| Код диагноз по МКБ-10                    | Должно быть заполнено                                        | Код диагноза. В соответствии с международной классификацией болезней |
| Название диагноза по МКБ                 | Должно быть заполнено                                        | Название диагноза. В соответствии с международной классификацией болезней |

### СОУТ

Требования к формату импортируемого файла для загрузки данных по оценки условий труда:

| Поле           | Требование                                                   | Комментарий                                                  |
| -------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| СНИЛС          | Одинадцатитизначная комбинация цифр, разделенных дефисом. Должно быть заполнено | Одинадцатитизначная комбинация цифр, разделенных дефисом. Пример: **XXX-EEE-YYY ZZ** |
| Факторы рисков |                                                              | Наименование вредных или опасных производственных факторов в соответствии с приказом Минздрава России от 28.01.2021 N 29н |
